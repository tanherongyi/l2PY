...
class OAuthMethod():
    ...
    def weibo_nomal_path(self):
        path = current_app.config['WEIBO_AUTHORIZE_URL'] + '?client_id=' + current_app.config[
            'WEIBO_CLIENT_ID'] + '&response_type=code' + '&redirect_uri=' + 'http://www.l2py.com/weibo/oauth/callback'
        return path

    def weibo_nomal_userinfo(self):
        # 判断回调地址中是否拿到code
        if 'code' not in request.args:
        ...
        # 将access_token 传入'https://api.weibo.com/users/show?access_token='，发送GET请求，获取用户信息
        url = 'https://api.weibo.com/users/show?access_token=%s' % access_token
        r = requests.get(url)
        r.encoding = r.content
        result = r.json()
        return result
